...
class DashboardService
{
    ...
    private function getConfirmedBookings()
    {
        return Auth::user()->bookings()
            ->with(['tickets.show.concert.artist', 'tickets.seat'])
            ...
            ->map(function ($booking) {
                return $booking->tickets->map(function ($ticket) use ($booking) {
                    return [
                        ...
                        'concert' => $ticket->show->concert->artist->name,
    ...
    {
        return Show::with(['concert.location', 'concert.artist'])
            ->where('start', '>=', now())
            ->orderBy('start')
            ->take(5)
            ->get()
            ->map(function ($show) {
                return [
                    'id' => $show->concert->id,
                    'artist' => $show->concert->artist->toArray(),
